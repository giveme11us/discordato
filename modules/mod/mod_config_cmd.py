...
async def handle_mod_config(interaction, setting=None, action=None, value=None):
    ...
    if not setting:
        ...
        if mod_config.WHITELIST_ROLE_IDS:
            whitelist_roles_str = "\n".join([f"• <@&{role_id}>" for role_id in mod_config.WHITELIST_ROLE_IDS])
        else:
    ...
    if setting.lower() == "whitelist":
        ...
        if action.lower() == "view":
            # Format the whitelist role IDs
            if mod_config.WHITELIST_ROLE_IDS:
                whitelist_roles_str = "\n".join([f"• <@&{role_id}> (ID: {role_id})" for role_id in mod_config.WHITELIST_ROLE_IDS])
            else:
                whitelist_roles_str = "No whitelist configured"
            ...
            current_whitelist = list(mod_config.WHITELIST_ROLE_IDS)
            ...
            mod_config.WHITELIST_ROLE_IDS = current_whitelist
