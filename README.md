...
- **Link Reaction System**: Automatically adds a 🔗 link emoji to messages in specified category channels. Similar to the reaction forward system, but uses a different emoji and doesn't perform any action when users react to the message. This can be used to visually mark messages that contain links or references to other content.
